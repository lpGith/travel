...
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
  ...
  <body style="background:url('/images/123456789.jpg') no-repeat;background-color:#1d1d11">
    ...
    <div class="wrapper" style="background-color:#ddd; padding-left:8px;padding-right:8px">
      <p>当前位置:<a href="/zhoubian/zhulist/">&nbsp;酒店客栈</a>&nbsp;>&nbsp;{{ $list[0]->zhu_name }} <span style="font-size:15px;display:bload;float:right;margin-top:8px">发布日期:{{ date("Y年m月d日",$list[0]->zhu_addtime) }}</span></p>
      <hr>
      <h3 class="sub" style="display:blode;width:400px;height:60px;float:left">{{ $list[0]->zhu_name }}</h3>
      <section class="container clearfix">
        ...
        <div class="row">
          <div class="left-col">
            <img class="he_border2_img" src="/admin_zhoubian/{{ $list[0]->zhu_picname }}" alt="Image 01">
            <p style="text-indent:2em">{{ $list[0]->zhu_describe }}</p>
          </div>
          <div class="right-col">
            <span style="font-size:15px">此酒店{{$list[0]->zhu_dianji}}次被旅客查询</span>
            <div style="height: 282px;" class="pin-wrapper"><div style="width: 290px; left: 882.5px; top: 0px; position: fixed;" class="pinned note-container">
              <div class="note">
                <p>{{ $list[0]->zhu_name }}</p>
                <p>{{ $list[0]->zhu_location }}</p>
                <h4>☏ {{ $list[0]->zhu_phone }}(联系我们)</h4>
                <h4><span">{{ $list[0]->zhu_type }}起入住</span><img src="{{asset('/images/pl.png')}}" id="liuyan" title="点击评论" style="float:right;width:70px;cursor:pointer;"></h4>
              </div>
              <p>&nbsp;</p>
            ...
            layer.prompt({ formType: 2}, function(text){
                $ .ajax({
                    ...
                    success:function(data){
                        var comment = $('#abc');
                        comment.load('/zhoubian/zhuxiangqing/{{ $list[0]->id }} #abc');
