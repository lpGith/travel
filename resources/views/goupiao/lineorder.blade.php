...
<div class="order_box" id="order_box">
    <h1 class="order_title " style="color:blue"><span class="icon heart_icon mr10 vm"></span>{{$list->tuangou_name}}</h1>
    <div class="order_form">
	 <div class="field J_departCity">
            <label>产品编号：{{$list->tuangou_bianhao}}</label>   
        </div>
        <div class="field J_departCity">
            <label>跟团游出发地和目的地：{{$list->tuangou_chufa}}-->{{$list->tuangou_mudi}}</label>   
        </div>
       
		<div class="field J_departCity">
            <label  style="color:red;">团游票价：<span id="one_price">{{$list->tuangou_price1}}</span>元/间/天起/(儿童票：{{$list->tuangou_price2}}元/间/天起)</label>   
        </div>
        <div class="field J_touristCount">
			<label>人数：</label>
			<div class="input_number J_adultCount">	
				<label>成人数</label><button  id="button_num_id1" >-</button>
									<span id="num_id">0</span>
									<button  id="button_num_id2">+</button>
				<label>儿童数</label><button  id="button_num_id3" >-</button>
		 ...
		 <div class="field J_departDate">
            ...
            <div class="input_select w_350">
				...
				<form class="J_form" id="form_id" onsubmit="return doCheck()" action="/goupiaodopayline" method="post">
...
<div class="contact panel mt20" id="contact_box">
	...
	<div class="field email_tips ">       
         ...
         <p>· 出游人信息不完整会延误你的正常出游；因信息不完整、不准确造成的保险拒赔等问题，我司不承担相应责任。</p>       
			...
			<input class="field_input" type="hidden" name="pay_type" value="跟团游订票">
			<input class="field_input" type="hidden" name="pay_jingdianname" value="{{$list->tuangou_name}}">
...
<div class="contact panel mt25" id="contact_box">
    ...
    <div class="panel_body">
	<div class="field email_tips ">       
         <p>1.18岁以下未成年人需要至少一名家长或成年旅客全程陪同。
			2.70周岁（含）以上老年人预订出行，需与我司签订《健康申报表》,并有18周岁以上家属或朋友（因服务能力所限无法接待及限制接待的人除外）全程陪同出行。
			3.本团体报价是按照2成人入住1间房计算的价格，本产品不接受拼房,如您的订单产生单房，请在预订后续页面中选择单人房差，我司将向您收取相应的费用。由于12岁以下儿童费用为不占床、不含早餐之报价，若儿童需占床含早，请在后续预订页面的附加产品中选择单人房差可选项。	
		 </p>       
    </div>       
    </div>   
</div>
...
$("#button_num_id1").click(function(){
	...
	if(num<1){
		...
		if(num2<1){
			...
			$("#total_price").html(price*num2/2);
			$("#pay_total_id").val(price*num2/2);
		}
		...
		var price = $("#one_price").html();
		var num2 = $("#num_id1").html();
		$("#total_price").html(price*num+price*num2/2);
		$("#pay_total_id").val(price*num+price*num2/2);		
	}
...
$("#button_num_id4").click(function(){
	...
	$("#total_price").html(price*num/2+price*num2);
	$("#pay_total_id").val(price*num/2+price*num2);
});
//判断点击儿童减号时价格的控制
