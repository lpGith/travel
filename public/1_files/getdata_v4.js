...
+"</div>"
+"</div><%}%>"
+"</li><%}%>";var listImg=_.template(adImg,data.adArea);return listImg;}else{return"";}},getApp:function(){if(this.noProduct()){var app="<% if(data.appArea){ %><li style='height:42px;margin-top:20px;cursor:pointer;'>"
+"<% if(data.appArea.appUrl){ %><div class='rc_index'>"
+"<p class='rc_app_box'>"
...
+"</span>"
+"<div class='rc_content'>"
+"<a href='<%=data.appArea.appUrl %>' target='_blank'>"
+"<img src='<%=data.appArea.appImgUrl %>' alt='' />"
+"</a>"
+"</div>"
+"</div><%}%>"
+"</li><%}%>";var listApp=_.template(app,data.appArea);return listApp;}else{return"";}},reNoLogin_passport:function(){var str=window.location.host;var domain=str.split(".")[0];var iframeSrc='https://passport.tuniu.com/login/iframe?origin='+encodeURIComponent('http://www.tuniu.com/ssoConnect/Iframe?reload='+domain);var reNLogin2="<% if(data.islogin != 1) {%><div class='rc_common_box nologin' style='width:378px;padding:0px; border:none; outline:none;'>"
+"<iframe src='"+iframeSrc+"'  width='100%' height='352px' frameborder='0'>"
+"</iframe>"
+"</div><% } %>"
var listNoLogin=_.template(reNLogin2,data.myTuniu);return listNoLogin;},reNoLogin:function(){var str=window.location.host;var domain=str.split(".")[0];var iframeSrc='https://passport.tuniu.com/login/iframe?origin='+encodeURIComponent('http://www.tuniu.com/ssoConnect/Iframe?reload='+domain);var reNLogin2="<% if(data.islogin != 1) {%><div class='rc_common_box nologin' style='width:378px;padding:0px; border:none; outline:none;'>"
+"<iframe src='"+iframeSrc+"'  width='100%' height='352px' frameborder='0'>"
+"</iframe>"
+"</div><% } %>"
...
+"</dl>"
+"</div><% } %>"
var listNoLogin=_.template(reNLogin,data.myTuniu);return listNoLogin;},getPreL:function(){var preL="<li><a href='http://www.tuniu.com/u/club' target='_blank'><span class='right_icons tequan <%=name.userPrivilege_1 %>'></span></a></li>";var preLTmp="<%_.each(data.myTuniu.userPrivilege,function(name){%>"
+preL
+"<% }) %>";return preLTmp;},getMyTuniu:function(){var myTuniu="<li class='mytuniuArea' style='padding:10px 0;cursorpointer;'>"
+"<div class='rc_index'>"
+"<p class=''>"
+"<% if(data.islogin == 0){ %><a href='http://www.tuniu.com/main.php?do=user_change_picture' target='_blank'><span class='rc_icon rc_tuniu'></span></a><%}%>"
+"<% if(data.islogin == 1){ if(data.myTuniu.userHeadImgUrl){%><a href='http://www.tuniu.com/main.php?do=user_change_picture' target='_blank'><img src='<%=data.myTuniu.userHeadImgUrl %>' style='width:32px;height:32px;margin:4px;background:none;border-radius:50%;' /></a><%}}%>"
+"<% if(data.islogin == 1){ if(!data.myTuniu.userHeadImgUrl){%><a href='http://www.tuniu.com/main.php?do=user_change_picture' target='_blank'><img src='http://img3.tuniucdn.com/img/2014040901/user_center/g_touxiang.png' style='width:32px;height:32px;margin:4px;background:none;border-radius:50%;' /></a><%}}%>"
+"</p>"
+"<p class='rc_wd' id='lessThanHide' style='padding:0;'>我的途牛</p>"
+"</div>"
+"<div class='rc_box nopad'>"
...
+"<a href='http://www.tuniu.com/main.php?do=user_change_picture' target='_blank'>"
...
+this.reNoLogin()
+"</div>"
+"</li>";var listMyTuniu=_.template(myTuniu,data.myTuniu);return listMyTuniu;},getLineList:function(t){var lineList="<li>"
...
+this.reNoLogin()
+"</div>"
+" </li>";return _.template(myCollect,data.myCollect);}else{return"";}},getScoreList:function(){var scoreList="<div class='right_tagContent' id='rtagContent0'>"
...
+this.reNoLogin()
+"</div>"
+"</li>";return _.template(myScoreList,data.myScore);}else{return"";}},myOrderList:function(){var order_list="<%_.each(data.myOrder.list,function(name){ %><li>"
...
+"</dd>"
+"</dl>"
+"</li><%})%>";return order_list;},getOrder:function(){if(this.noProduct()){var orderList="<li class='hoverClick'>"
+"<div class='rc_index'>"
+"<p class='rc_topBot_b'>"
+"<span class='rc_icon rc_order'></span>"
+"</p>"
+"</div>"
+"<div class='rc_box nopad nobord rc_hover_event'>"
+"<div class='rc_content'>"
+"<p class='rc_des'>我的订单</p>"
+"</div>"
+"</div>"
+"<div class='rc_box rc_click_event nopad'>"
+"<span class='rc_arrow'><i class='triangle_border'><em></em></i></span>"
+"<% if(data.islogin == 1){ %><div class='rc_common_box'>"
+"<dl class='rc_double_col rc_mycol clearfix'>"
+"<dt><div class='rc_btn_bord'>订单类型</div></dt>"
+"<dd><a href='http://www.tuniu.com/u/order' class='rc_g_color' target='_blank'>更多订单 &gt;</a></dd>"
+"</dl>"
+"<ul class='clearfix rc_two_col'>"
+"<li>待付款<span class='rc_y_color'>(<%= data.myOrder.need_pay%>)</span></li>"
+"<li>待点评<span class='rc_y_color'>(<%= data.myOrder.need_comment%>)</span></li>"
+"</ul>"
+"<dl class='rc_double_col rc_mycol clearfix'>"
+"<dt><div class='rc_btn_bord'>订单状态</div></dt>"
+"<dd>"
+"</dd>"
+"</dl>"
+"<!-- rc_order_box start -->"
+"<div class='rc_order_box'>"
+"<% if(!data.myOrder.list[0].id){ %><img src='http://img4.tuniucdn.com/img/20140914/rightcommon/nothing.jpg' alt='' /><%}%>"
+"<% if(data.myOrder.list[0].id){ %><ul class='rc_order_lists'>"
+this.myOrderList()
+"</ul><%}%> "
+"</div>"
+"</div><% } %>"
+this.reNoLogin()
+"<% if(data.myOrder.list.preAdImg){ %><a href='<%=data.myOrder.list.prdAdUrl %>' target='_blank'>"
+"<img src='<%=data.myOrder.list.preAdImg %>' alt='' />"
+"</a><%}%>"
+"</div>"
+"</li>";return _.template(orderList,data.myOrder);}else{return"";}},myListInfor:function(){var listNum="<% if(data.islogin == 1 && data.myMessage.sum>0){ _.each(prd,function(name){ %><li>"
...
+"<% if(data.islogin != 1){ %>"
+this.reNoLogin()
+"<%}%>";return _.template(numOfList,data.myMessage);},getCompList:function(){var compList="<li>"
...
+"</ul>"
+"<dl class='rc_double_col rc_mycol clearfix'>"
+"<dt><div class='rc_btn_bord'>如何获得旅游券</div></dt>"
+"<dd>"
+"</dd>"
+"</dl>"
+"<ul class='rc_collect_box'>"
+this.getArchList()
+"</ul>"
+"</div><%}%>"
+this.reNoLogin()
+"</div>"
+"</li>";return _.template(myArchList,data.myGift);}else{return"";}},getParter:function(){var c_name="tuniu_partner";if(document.cookie.length>0){var c_start=document.cookie.indexOf(c_name+"=");if(c_start!=-1){c_start=c_start+c_name.length+1;var c_end=document.cookie.indexOf(";",c_start);if(c_end==-1){c_end=document.cookie.length;}
return unescape(document.cookie.substring(c_start,c_end));}}
return"";},getPhone:function(){var page_type=document.getElementById("page_type").value;if(!this.noProduct()){var tuniuNum=$(".tuniu_400_num").html();var isShowPhone=$("#phow_show");var isShowPhoneVal=isShowPhone.val();if(page_type>5000000&&page_type<8000000){isShowPhoneVal=1;}
...
+"</div>"
+"</li>"
return phone;}else{return"";}}else{return"";}},getKefu:function(){var kefu='';var is_kefu=document.getElementById("kefu_show");var listNoLogin='<p class="rc_des">在线客服</p>';var localTime=new Date().getHours().toLocaleString();var numTime=parseInt(localTime,10);var kefuType='live800';if((numTime>=8&&numTime<23)){if(!this.noProduct()){kefu="<li class='' id='onlineKefu' onclick=\"_gaq.push(['_trackEvent','在线客服按钮_1','','']);\">"
...
var href=location.href;var re=/cruise/;if(!is_kefu&&!re.test(href)){kefu="";return kefu;}
if(/visa/.test(href)){kefu="";return kefu;}
if(true){var str=window.location.host;var domain=str.split(".")[0];var productId=$('#route_id_tmp').val();var title=$('title').text();var entranceUrl=location.pathname;$.ajax({dataType:'jsonp',jsonpCallback:'success_jsonpCallback',jsonp:'cb',cache:false,timeout:10000,url:'http://m.tuniu.com/chat/customer/IsShowCustomerIconByJsonp?c={%22ct%22:100}&d={%22productType%22:3,%22productId%22:%22'+productId+'%22}',success:function(response){if(response.success){if(response.data.status){var productType=response.data.productType?response.data.productType:1;$(document).off('click.loginPassport','#onlineKefu, .online_service, .online_btn').on('click.loginPassport','#onlineKefu, .online_service, .online_btn',function(e){$('#onlineKefu').trigger('click.openKefuWindow');}).off('click.closeLoginPassport').on('click.closeLoginPassport',function(e){if(!$(e.target).hasClass('online_btn')&&$(e.target).parents('.online_btn').length<1&&!$(e.target).hasClass('u_order_tip')&&$(e.target).parents('.u_order_tip').length<1){$('.order_now .rc_box').remove();$('.u_order_tip .rc_box').remove();}});kefuType='tuniuKefu';if($('#onlineKefu').length>0){$('#onlineKefu #live800').attr('id',kefuType);}else{var kefu="<li class='' id='onlineKefu' onclick=\"_gaq.push(['_trackEvent','在线客服按钮_1','','']);\">"
+"<div class='rc_index' style='cursor:pointer;'>"
+"<p class='rc_topBot_b' style='border-bottom:none;'>"
+"<a href='javascript:void(0)' id='"+kefuType+"'><span class='rc_icon rc_online'></span></a>"
+"</p>"+"<p class='rc_wd' style='padding:0;'>在线客服</p>"
...
$(document).on('click.openKefuWindow','#onlineKefu',function(e){var kefuUrl='http://dynamic.m.tuniu.com/livechat?productId='+productId+'&title='+title+'&productType='+productType+'&entranceUrl='+entranceUrl;kefuUrl=encodeURI(kefuUrl);window.open(kefuUrl,'在线客服','width=640, height=515, top=100, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');})}else{}}else{}}})}
...
+"</div>"
+"</div>"
+"</li><%}%>";return _.template(Saoma,data.myCode);}else{return"";}},addToCollect:function(){var page_type=document.getElementById("page_type").value;if(page_type==92000)return"";if(page_type==32000)return"";if(!this.noProduct()){var addToCollectList="<li class='' id='doAddToCollect'>"
+"<div class='rc_index'>"
+"<p class='rc_topBot_b'>"
...
var comPareList=document.getElementById("comPareList");var base_url="//www.tuniu.com";var url=base_url+'/yii.php?r=toursAjax/getInfo/routeIds/'+routeIds+'&jsoncallback=?';$.getJSON(url,function(r){compName=r;var compModuel=[{"id":"","showName":""}]
...
s_tag+="</ul>";right_tagContent.after(s_tag);$("#right_tags").find("li").each(function(i,n){$(n).click(function(){$(n).siblings().removeClass("selectTag").end().addClass("selectTag");right_tagContent.find(".right_tagContent").removeClass("selectTag").eq(i).addClass("selectTag");});});},createVCode:function(){var rightCommonUl=$("#rightCommonUl").find("li");var rd=Math.random();rightCommonUl.each(function(i,n){$(n).find('.identify_img').attr('src','/identify.php?flag=100&cache='+rd);$(n).find(".change_img").click(function(){var rd_2=Math.random();$(n).find('.identify_img').attr('src','/identify.php?flag=100&cache='+rd_2);});})},changeVcode:function(){var rightCommonUl=$("#rightCommonUl").find("li");var rd=Math.random();rightCommonUl.each(function(i,n){$(n).find('.identify_img').attr('src','/identify.php?flag=100&cache='+rd);})},addLive800:function(){var live800=$("#live800");var onlineKefu=$("#onlineKefu");if(!live800)return"";var groupid=$("#kefu_show").attr("groupid");onlineKefu.bind("click",function(){getData.Live800(groupid,560,480);});},htmlspecialchars:function(str){str=str.replace('&','&amp;');str=str.replace('<','&lt;');str=str.replace('>','&gt;');str=str.replace('"','&quot;');str=str.replace(' ','&nbsp;');return str;},Live800:function(groupid,iWidth,iHeight){if(!document.getElementById('live800'))return;if(!document.getElementById('live800').value){var tuniuTrakerNew=_tat.getTracker();tuniuTrakerNew.setPageName("客服在线帮助页面");tuniuTrakerNew.trackEvent('live800');tuniuTrakerNew=null;}
