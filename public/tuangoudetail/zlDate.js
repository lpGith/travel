...
var pickerEvent = {
    Init: function (elemid) {
        ...
        var html = '<div id="calendar_choose" class="calendar1" style="display: block; position: absolute;">'
        html += htmlObj.left;
        html += htmlObj.header;
	...
	//	document.getElementById("picker_today").onclick = pickerEvent.getToday;
        ...
        document.getElementById("calendar_choose").style.top  = getAbsoluteTop(elemid)-10+"px";
...
var pickerHtml = {
    getHead: function () {
        var head = '<ul class="calendar_num basefix"><li class="bold"  style="width:90px;border-left:none">日</li><li style="width:90px;">一</li><li  style="width:90px;">二</li><li  style="width:90px;">三</li><li  style="width:90px;">四</li><li  style="width:90px;">五</li><li class="bold"  style="width:89px;">六</li></ul>';
        htmlObj.header = head;
    },
    getLeft: function () {
        var left = '<div class="calendar_left pkg_double_month"><p class="date_text">' + obj.year + '年' + obj.month + '月</p><a href="javascript:void()" title="上一月" id="picker_last" class="pkg_circle_top">' + obj.year + '年' + (obj.month - 1) + '月</a><a href="javascript:void()" title="下一月" id="picker_next" class="pkg_circle_bottom ">' + obj.year + '年' + (obj.month + 1) + '月</a></div>';
        htmlObj.left = left;
    },
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                var children = commonUtil.getChildren((i - c));
                var priceid = commonUtil.getPriceId((i - c));
                var priceStr = "";
                var classStyle = "";
                if (price != -1) {
                ...
                if (obj.year == new Date().getFullYear() && obj.month == new Date().getMonth() + 1 && i - c == new Date().getDate()) {
                    html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" children="' + children + '" priceid="' + priceid + '"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
                }
                else {
                    html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '"children="' + children + '" priceid="' + priceid + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
                }
                if (index == 6) {
...
var commonUtil = {
    ...
    getChildren: function (day) {
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].Date == dt) {
                return obj.priceArr[i].Children;
            }
        }
        return -1;
    },
    getPriceId: function (day) {
        var dt = obj.year + "-";
        if (obj.month < 10) {
            dt += "0" + obj.month;
        }
    ...
    chooseClick: function (sender) {
        ...
        var children = sender.getAttribute("children");
        var priceid = sender.getAttribute("priceid");
